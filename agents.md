# Agents

Refer to @docs/design.md ALWAYS when creating components.

Refer to @.impeccable.md ALWAYS when touching any UI.

Refer to @docs/development.md for information about development.

Run required check commands from [package.json](package.json) after completion of work.

We are using Bun as the runtime.

Use commitlint style commit messages.

There may be other agents actively making changes at any time.

Only commit YOUR changes, do NOT use `git add .`.